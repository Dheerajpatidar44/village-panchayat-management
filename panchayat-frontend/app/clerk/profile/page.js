...
import { User, Mail, Phone, Edit, Key, LogOut, Loader2 } from "lucide-react";
import { api } from "@/lib/api";
import { useRouter } from "next/navigation";
import { Button } from "@/components/ui/Button";

// Clerk and Citizen profiles use the same structure as Admin for now due to shared backend /auth/me
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage your account credentials and personal details</p>
        </div>
      </div>
         ...
         <Card className="md:col-span-2">
            ...
            <CardContent className="space-y-6">
               ...
               <div className="pt-6 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Button variant="outline" className="w-full gap-2 text-rose-500 hover:text-rose-600 hover:bg-rose-50 border-rose-100" onClick={() => { localStorage.clear(); router.push('/login'); }}><LogOut className="w-4 h-4" /> Logout</Button>
               </div>
            </CardContent>
         </Card>
      </div>
